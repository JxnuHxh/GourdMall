      ...
      <el-container class="shop-cart-wrap">
        ...
        <el-main style="padding: 0px">
          ...
          <el-row class="shop-good" v-for="(item, index) in this.shopCart.goods" :key="index">
            ...
            <el-col :span="9">{{item.name}} {{item.total}}</el-col>
            <el-col :span="5">

                <change-num :nextNum='item.total' v-if="item.total === shopCart.goods[index].total" :maxNum=100 :miniNum = 1 :index=index :tip='""' @lastNum="test"></change-num>
            </el-col>
        ...
        <el-footer class="shop-cart-footer">
          <el-row id="wrap-footer">
            ...
            <el-col :span="7">已选中{{this.shopCart.total}}件商品</el-col>
            <el-col :span="4">共￥{{this.shopCart.sum}}元</el-col>
            <el-col :span="5">
...
export default {
  ...
  methods: {
    ...
    delChecked () {
      console.log('删除选中的')
      const list = this.shopCart.goods.filter(item => !item.checked)
      this.shopCart.goods = list
      this.sum()
    },
    choose (index) {
      // console.log(this.shopCart.goods[index].checked)
      this.shopCart.goods[index].checked = !this.shopCart.goods[index].checked
      // console.log(this.shopCart.goods[index].checked)
      this.sum()
    },
    delGoods (index) {
      ...
      const list = this.shopCart.goods.filter(item => (item !== this.shopCart.goods[index]))
      this.shopCart.goods = list
    ...
    sum () {
      ...
      this.shopCart.goods.forEach(
        (item, index, arr) => {
          if (item.checked) { this.shopCart.sum += item.total * item.price }
        }
      )
    },
    handleScroll () {
