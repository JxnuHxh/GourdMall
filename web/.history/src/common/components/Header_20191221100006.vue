  ...
  <el-header style="border-radius: 0px; background: #333333">
    <el-row >
      <el-col :offset="5" :xs="24" :sm="24" :md="24" :lg="14" :xl="14"  id="nav-bar">
        ...
        <!-- 用户下拉菜单， 只在登录后才显示-->
        <el-dropdown v-if="loginState && this.$route.name !== 'User'" class="right-links">
          <span class="el-dropdown-link">
            {{userMessage.nickname}}<i class="el-icon-arrow-down el-icon--right"></i>
          </span>
        ...
        <el-link v-if="!loginState" :underline="false" class="right-links" @click="Register()">{{loginState}}</el-link>
        <el-link v-if="!loginState"  :underline="false" class="right-links" @click="Login()">登录</el-link>
