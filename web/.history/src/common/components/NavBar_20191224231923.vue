    ...
    <el-row :gutter="20">
      <el-col :span="14"  :offset="5">
        <div id="nav-bar-links"  >
          <el-link :underline="false" :disabled = "index === '0'" @click="change('0')"><div > <router-link style="text-decoration: none;" :to="{name:'Home',params:{action:'OrderList'}}"> 首页 </router-link> </div></el-link>
          <span v-for="item in cateGories" v-bind:key="item.cid">
            <el-link :underline="false" :disabled = "index === item.cid" @click="change(item.cid)">
              <div @mouseenter="naventer(item)">{{item.cname}}</div>
            </el-link>
          </span>
