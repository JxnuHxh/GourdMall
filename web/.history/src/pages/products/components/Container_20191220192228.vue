...
export default {
  ...
  computed: {
    ...
    showIndex () {
      if (this.pageSize >= this.productList.total) {
  ...
  methods: {
    ...
    sortDefault () {
      this.sortByKeyAsc(this.productList.data, 'index')
    },
    sortPriceAsc () {
      this.sortByKeyAsc(this.productList.data, 'price')
    },
    sortPriceDesc () {
      this.sortByKeyDesc(this.productList.data, 'price')
    },
